...
| `client/src/pages/AdminPanelPage.tsx` | Replaces match logo URL fields with actual image-file pickers for home team, away team, and league images. Selected PNG/JPG/WEBP/SVG files are previewed, removable, limited to 2 MB each, and encoded into the existing logo payload fields when the match is created or scheduled. |
| `client/src/pages/SuperAdminPage.tsx` | Adds the dedicated Super Bet Super Admin control centre. It is guarded by the exact `SUPER_ADMIN` role and covers dashboard metrics, administrators, commission rates, admin/user funds, user lifecycle, user deposits, transactions, Binance/bank/simple deposit queues, affiliate withdrawals, payout requests, wallet withdrawals, commission analytics, upgrade chats, and audit log. |
| `client/src/lib/api.ts` | Adds the complete Super Admin API surface from the reference ZIP, including admin commission/funds, user status/funds/deposits, country commission reports, deposit queues, and wallet withdrawal settlement/failure actions. |
| `client/src/App.tsx` | Adds the `/super-admin` route. |
| `client/src/components/AccountCenter.tsx` | Adds a `SUPER_ADMIN`-only **Super Bet Super Admin** account link while retaining the regular Admin Centre for `ADMIN` and `SUPER_ADMIN`. |
...
Match images are selected from the administrator's device; there are no logo URL fields. The current backend/reference projects do not expose a dedicated media-upload endpoint, so the selected file is sent as a data-URL string through the backend's existing `homeLogo`, `awayLogo`, and `leagueLogo` fields. The UI validates file type and limits each image to 2 MB before submission.

### Super Bet Super Admin
