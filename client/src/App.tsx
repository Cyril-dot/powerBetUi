...
function Header({ onMenu }: { onMenu: () => void }) {
  ...
  return <>
    ...
    <header className="header"><div className="wrap header-inner"><Link href="/" className="brand" aria-label="Super Bet home"><img src={mark} alt="Super Bet" /></Link><nav id="primary-navigation" className="main-nav">{nav.map(([label, href]) => <Link key={href} href={href} className={location === href ? "active" : ""} onClick={() => document.body.classList.remove("menu-open")}>{label}{label === "Live" && <i className="live-dot"/>}</Link>)}</nav><div className="header-actions"><button className="icon-button" aria-label="Search" onClick={()=>window.location.href="/search"}><Search size={17}/></button>{token ? <><Link href="/wallet" className="header-balance">{balance !== null ? `GHS ${balance.toLocaleString("en-GH", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` : "GHS 0.00"}</Link><Link href="/deposit" className="header-deposit"><Plus size={14} /> Deposit</Link><Link href="/account" className="login-link">My account</Link></> : <><Link href="/login" className="login-link">Log in</Link><Link href="/register" className="header-join-button">Join now</Link></>}</div></div></header>
  </>;
}
