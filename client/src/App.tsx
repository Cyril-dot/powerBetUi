...
import { Bell, ChevronDown, ChevronRight, CircleHelp, Clock3, Copy, CreditCard, Flame, Gamepad2, Gift, Headphones, Info, Layers3, LayoutGrid, Minus, MoreHorizontal, Play, Plus, Radio, ScanBarcode, Search, ShieldCheck, Sparkles, Ticket, Trophy, UserRound, WalletCards, X, Zap, Lock } from "lucide-react";
...
const mark = "/superbet-pixel-notch.svg";
...
function Header({ onMenu }: { onMenu: () => void }) {
  ...
  return <>
    <div className="utility"><div className="wrap utility-inner"><Link href="/" aria-label="SUPER BET home" className="utility-brand"><span className="utility-wordmark">SUPER BET</span></Link><span className="utility-links"><Link href="/support">Support</Link>{token ? <><Link href="/bets">Bet history</Link><Link href="/account">My account <ChevronDown size={12}/></Link></> : <><Link href="/login">Log in</Link><Link href="/register">Join now</Link></>}</span></div></div>
    <header className="header"><div className="wrap header-inner"><Link href="/" className="brand brand-logo" aria-label="SUPERBET home"><img src={mark} alt="SUPERBET" /></Link><nav id="primary-navigation" className="main-nav">{nav.map(([label, href]) => <Link key={href} href={href} className={location === href ? "active" : ""} onClick={() => document.body.classList.remove("menu-open")}>{label}{label === "Live" && <i className="live-dot"/>}</Link>)}</nav><div className="header-actions"><button className="icon-button" aria-label="Search" onClick={()=>window.location.href="/search"}><Search size={17}/></button>{token ? <><div className="header-wallet-group"><Link href="/wallet" className="header-balance"><span className="balance-label">Balance</span><strong>{balance !== null ? `GHS ${balance.toLocaleString("en-GH", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` : "GHS 0.00"}</strong></Link></div><Link href="/account" className="login-link">My account</Link></> : <><Link href="/login" className="login-link">Log in</Link><Link href="/register" className="header-join-button">Join now</Link></>}</div></div></header>
  </>;
}
...
function AdminGameCard({ match, picks, onPick, choose, hasDraw }: { match: EnrichedMatch; picks: Pick[]; onPick: (p: Pick) => void; choose: (m: EnrichedMatch, selection: "1" | "X" | "2", odd: number) => void; hasDraw: (m: EnrichedMatch) => boolean }) {
  const homeLogo = adminFeaturedLogo(match.homeTeam, "home");
  const awayLogo = adminFeaturedLogo(match.awayTeam, "away");
  const [homeReady, setHomeReady] = useState(false);
  const [awayReady, setAwayReady] = useState(false);
  const [blocked, setBlocked] = useState(false);
  useEffect(() => {
    let active = true;
    setHomeReady(false); setAwayReady(false); setBlocked(false);
    const preload = (src: string, ready: (value: boolean) => void) => {
      const image = new Image();
      image.onload = () => { if (active) ready(true); };
      image.onerror = () => { if (active) { setBlocked(true); ready(false); } };
      image.src = src;
    };
    preload(homeLogo, setHomeReady);
    preload(awayLogo, setAwayReady);
    return () => { active = false; };
  }, [homeLogo, awayLogo]);
  if (blocked || !homeReady || !awayReady) return null;
  const draw = hasDraw(match);
  const values: Array<["1" | "X" | "2", string, number]> = [["1", "HOME", match.oddsMap?.home ?? 0], ...(draw ? [["X", "DRAW", match.oddsMap?.draw ?? 0] as ["X", string, number]] : []), ["2", "AWAY", match.oddsMap?.away ?? 0]];
  return <article className="admin-game-card" key={match.id}><div className="admin-game-top"><span className="admin-game-league">{match.league || match.sport || "Admin special"}</span></div><Link href={`/match/${match.id}?sport=${encodeURIComponent(match.sport ?? "football")}&admin=1`} className="admin-game-fixture"><div className="admin-game-team"><img src={homeLogo} alt={`${match.homeTeam} logo`} loading="eager" onError={() => setBlocked(true)} /><b>{match.homeTeam}</b></div><div className="admin-game-mid"><strong>{isMatchLive(match) ? "LIVE" : "VS"}</strong><small>{formatKickoff(match.kickoffAt)}</small></div><div className="admin-game-team"><img src={awayLogo} alt={`${match.awayTeam} logo`} loading="eager" onError={() => setBlocked(true)} /><b>{match.awayTeam}</b></div></Link><div className="admin-game-odds">{values.map(([label, name, value]) => { const selected = picks.some((p) => p.id === match.id && p.market === "1X2" && p.selection === label); return <button type="button" key={label} className={`admin-game-odd${selected ? " selected" : ""}`} disabled={!value || value <= 0} onClick={() => choose(match, label, value)}><span>{name}</span>{value ? value.toFixed(2) : "—"}</button>; })}</div></article>;
}
...
function AdminFeaturedGames({ picks, onPick }: { picks: Pick[]; onPick: (p: Pick) => void }) {
  ...
  return <section className="admin-featured-games"><style>{`.admin-featured-games{margin:16px 0 20px;border:1px solid rgba(109,228,157,.28);border-radius:14px;background:linear-gradient(135deg,rgba(12,31,27,.96),rgba(9,17,24,.98));overflow:hidden;box-shadow:0 10px 24px rgba(0,0,0,.14)}.admin-featured-head{display:flex;align-items:center;justify-content:space-between;gap:10px;padding:12px 14px;border-bottom:1px solid rgba(112,226,157,.16)}.admin-featured-kicker{display:flex;align-items:center;gap:6px;color:#7fe8a5;font-size:8px;font-weight:900;letter-spacing:.1em;text-transform:uppercase}.admin-featured-head h2{margin:4px 0 0;color:#f3fff7;font-size:15px;line-height:1.1}.admin-featured-head p{margin:4px 0 0;color:#8da8a0;font-size:9px}.admin-featured-count{border:1px solid rgba(119,231,160,.42);background:#153a28;color:#b9f5c9;border-radius:999px;padding:5px 8px;font-size:8px;font-weight:900}.admin-featured-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px;padding:10px}.admin-game-card{position:relative;min-width:0;padding:10px;border:1px solid #263c43;border-radius:11px;background:linear-gradient(145deg,#111f27,#0c151b);transition:transform .18s,border-color .18s,box-shadow .18s}.admin-game-card:hover{transform:translateY(-1px);border-color:#62dd94;box-shadow:0 7px 16px rgba(33,168,91,.12)}.admin-game-top{display:flex;justify-content:space-between;align-items:flex-start;gap:6px}.admin-game-league{color:#8ca7a2;font-size:8px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.admin-game-tags{display:none}.admin-game-fixture{display:grid;grid-template-columns:1fr auto 1fr;align-items:center;gap:6px;margin:10px 0 8px}.admin-game-team{text-align:center;min-width:0}.admin-game-team img{width:34px;height:34px;object-fit:contain;display:block;margin:0 auto 4px;border-radius:9px}.admin-game-team b{display:block;color:#f0f7f2;font-size:9px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.admin-game-mid{text-align:center;color:#829c99;font-size:8px}.admin-game-mid strong{display:block;color:#b9f0c8;font-size:10px}.admin-game-mid small{display:block;margin-top:3px;font-size:8px}.admin-game-odds{display:grid;grid-template-columns:repeat(3,1fr);gap:4px}.admin-game-odd{border:1px solid #2d4850;background:#14262d;color:#bdeed0;border-radius:6px;padding:5px 3px;cursor:pointer;font-size:9px;font-weight:900}.admin-game-odd span{display:none}.admin-game-odd:hover,.admin-game-odd.selected{background:#235d3b;border-color:#6ee6a0;color:#fff}.admin-game-odd:disabled{opacity:.45;cursor:not-allowed}.admin-game-special{display:none}@media(max-width:720px){.admin-featured-grid{grid-template-columns:1fr}.admin-featured-head{align-items:flex-start;flex-direction:column}}`}</style><div className="admin-featured-head"><div><div className="admin-featured-kicker"><Sparkles size={11}/> Featured matches</div><h2>Admin-created fixtures</h2><p>Exclusive fixtures created by the Super Bet operations team.</p></div><span className="admin-featured-count">{matches.length} active</span></div><div className="admin-featured-grid">{matches.map((m) => <AdminGameCard key={m.id} match={m} picks={picks} onPick={onPick} choose={choose} hasDraw={hasDraw} />)}</div></section>;
}
...
function DepositPage(){ return <SimplePage title="Deposit center locked" kicker="Wallet funding"><section className="panel simple-card" style={{textAlign:"center"}}><Lock size={42} style={{color:"var(--gold-hi)",marginBottom:12}} /><h2>Deposits are not active</h2><p>Deposit funding is currently locked and unavailable. Please contact support if you need help with an existing payment.</p><Link href="/support" className="gold-button">Contact support <ChevronRight size={15} /></Link></section></SimplePage> }
