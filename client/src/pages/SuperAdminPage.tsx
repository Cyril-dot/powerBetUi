...
function CommissionAnalytics() {
  ...
  return <div className="sa-stack">
    ...
    <div className="sa-analytics-stat-grid">
      ...
      <div className="sa-analytics-stat"><small>{range === "daily" ? "Commission owed for selected day" : range === "live" ? "Commission owed today" : "Platform commission"}</small><strong>{money(totalCommission)}</strong><span>{range === "daily" ? settlementDate : range === "live" ? "Current day" : "Authoritative backend total"}</span></div>
      <div className="sa-analytics-stat"><small>Administrators shown</small><strong>{selected ? 1 : visibleAdminCards.length}</strong><span>{selected ? "Selected administrator" : normalizedSearch ? `Matching “${adminSearch.trim()}”` : "Zero-activity admins included"}</span></div>
    </div>
