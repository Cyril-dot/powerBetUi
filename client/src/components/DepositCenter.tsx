...
export default function DepositCenter() {
  ...
  const copyPaymentNumber = async () => {
    try {
      await navigator.clipboard.writeText("0536676400");
      setCopied(true);
      window.setTimeout(() => setCopied(false), 1800);
    } catch {
      setError("Could not copy the number. Please copy 0536676400 manually.");
    }
  };
          ...
          <form className="deposit-panel" onSubmit={submit}>
            <div className="deposit-panel-heading">
              <div>
                ...
                <p>
                  Pay to the MTN number below, then submit the same details shown
                  on your receipt or transaction history.
                </p>
              </div>
              <Smartphone size={22} />
            </div>
            <section className="deposit-payment-card" aria-label="MTN payment details">
              <div className="deposit-payment-card-head">
                <div>
                  <span className="deposit-payment-label">PAYMENT DETAILS</span>
                  <strong>MTN Mobile Money</strong>
                </div>
                <span className="deposit-network-pill">MTN</span>
              </div>
              <div className="deposit-payment-row">
                <div><span>Send payment to</span><strong>0536676400</strong></div>
                <button type="button" className="deposit-copy-button" onClick={() => void copyPaymentNumber()} aria-label="Copy MTN payment number">
                  {copied ? <><Check size={14} /> Copied</> : <><Copy size={14} /> Copy number</>}
                </button>
              </div>
              <div className="deposit-payment-name"><span>Account name</span><strong>ISAAC SENIOR APPIAH</strong></div>
            </section>
            <div className="deposit-instruction-notice" role="note">
              <Smartphone size={17} />
              <div><strong>How to pay with MTN</strong><p>Dial <b>*170#</b>, choose <b>Send Money</b>, enter <b>0536676400</b>, and pay the amount you want to deposit. After payment, upload your screenshot and submit the proof below.</p></div>
            </div>
            {error && (
